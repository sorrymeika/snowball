...
(function (factory) {
    ...
    var param = {
        error: {
            longNumber:
                '长数字存在精度问题，请使用字符串传值！ Long number is not allowed, because the precision of the Number In JavaScript.'
        }
    };
    ...
    var util = {
        ...
        // 生日码检查
        // checkBirth added by xiaohj on 2016.9.20
        checkBirth: function (birth) {
            ...
            if (birth.length == 8) {
                ...
                if (
                    temp_date2.getFullYear() != parseFloat(year) ||
                    temp_date2.getMonth() != parseFloat(month) - 1 ||
                    temp_date2.getDate() != parseFloat(day)
                ) {
                    return false;
                } else {
                    return true;
    ...
    _IDValidator.prototype = {
        isValid: function (id) {
            ...
            // 正则检测 added by xiaohj on 2012.9.20
            var matchString = /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)?$/i;
            ...
            //  }
            //  if(!matchString.test(code.body)) {
            //    return false;
            //  }
            if (!matchString.test(id)) {
                return false;
            }
            ...
            var birth =
                code.type === 18
                    ? code.body.slice(6, 14)
                    : code.body.slice(6, 12);
            // var order = code.body.slice(-3);

            // 地区检测 生日检测
            if (!(util.checkAddr(addr, GB2260) && util.checkBirth(birth))) {
            ...
            checkBit =
                typeof checkBit === 'number' ? checkBit.toString() : checkBit;
        ...
        makeID: function (isFifteen) {
            ...
            yr = '19' + yr;
            var body =
                addr + yr + mo + da + util.str_pad(util.rand(999, 1), 3, '1');
